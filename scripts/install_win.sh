...
standaloneInstall(){
    ...
    docker compose exec -w //usr/bin/ msa-dev bash -c  "./install_libraries.sh $(getLibOptions)"
    if [ $ccla = true ] ; then
        echo "Installing CCLA libraries and Blueprints"
        docker compose exec -w //usr/bin/ msa-dev bash -c  "./install_libraries.sh ccla"
    fi
